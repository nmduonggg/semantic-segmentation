...
def main(cfg, gpu, save_dir):
    ...
    wandb.init(
        project='CMC-TSR',
        group=f"segformer_rc_{cfg['MODEL']['BACKBONE']}",
        entity='nmduonggg',
        name=f"{cfg['MODEL']['NAME']}_{cfg['TRAIN']['IMAGE_SIZE']}"
    )
    ...
    for epoch in range(epochs):
        ...
        train_loss /= iter+1
        track_dict['train_loss'] = train_loss
        ...
        if (epoch+1) % train_cfg['EVAL_INTERVAL'] == 0 or (epoch+1) == epochs:
            ...
            track_dict['val_miou'] = miou
            if miou > best_mIoU:
                track_dict['best_miou'] = miou
                best_mIoU = miou
